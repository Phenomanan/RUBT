 ...
 *
 * @author Manan
 * This thread downloads pieces from the peer
 */
class Downloader extends Thread{
    ...
    public void run(){
        ...
        while(RUBTClient.remaining>0 && x<500){//Our main download loop
            ...
            try {
                ...
                System.out.println("Thread download failed."); // FLAG
                e1.printStackTrace();
            }
            x++;
        ...
        RUBTClient.decrementActiveThreads();
    }
	...
	public void download() throws IOException{
        ...
        int x;
        final ReentrantLock lock = new ReentrantLock(); // This is to lock methods so that they are not accessed by two threads simultaneously
        ...
        //System.out.println("Input/Output streams successfully opened."); // Flag
        
        
        
        byte[] peerLine; // Will be used for handshake and message
        peerLine = RUBTClient.handshake(info.info_hash.array(), b); // Set it to equal the
        // handshake
        try {
        ...
        //System.out.println("Our SHA-1: " + our_SHA + " , Peer's SHA-1: " + response_SHA);
        if (!response_SHA.equals(our_SHA)) {
            ...
            //System.out.println("Handshake successful, SHA-1 Hash confirmed.");
        }
        ...
        while(peerInput.available() != 0 || tries>0){
            ...
            if(x > 0){
                ...
                if(message=="unchoke"){
                    ...
                    lock.lock(); // FLAG
                    temp = RUBTClient.whatNext();//we use whatNext() to generate the request message
                    lock.unlock();
                    peerOutput.write(temp);
                    requesting = true;
                    //need to wait for response
                    //I think we also have to wait one interval time between requests
                }
 ...
 *
 * @author Manan
 * This thread uploads pieces to the asking peer
 */
...
public class RUBTClient {
    ...
    //these global variables are for convenience when sending the messages to the peer
    
    static byte[] KEEP_ALIVE = new byte[] { (byte) 0x00, (byte) 0x00,
    (byte) 0x00, (byte) 0x00 };
    private static byte[] CHOKE = new byte[] { (byte) 0x00, (byte) 0x00,
    (byte) 0x00, (byte) 0x01, (byte) 0 };
    static byte[] UNCHOKE = new byte[] { (byte) 0x00, (byte) 0x00,
    (byte) 0x00, (byte) 0x01, (byte) 1 };
    static byte[] INTERESTED = new byte[] { (byte) 00, (byte) 00,
    (byte) 00, (byte) 01, (byte) 2 };
    private static byte[] UNINTERESTED = new byte[] { (byte) 0x00, (byte) 0x00,
    (byte) 0x00, (byte) 0x01, (byte) 3 };
    ...
    /**
     *
     * @param args - takes in .torrent file and .mov file name
    ...
    IOException {
        ...
        if (args.length != 2) {
            System.out
            .println("RUBTClient requires two arguments: <.torrent file, .mov file name");
        }
        ...
        download(info); // Gets peers, then creates downloader and uploader threads
    ...
    /*
     * NOTE: byte[] input MUST be a 4-length byte[]
     * this method encodes the int value as hex and stores the hex value in the byte array
     * @author Mehul
     */
    private static byte[] setHex(byte[] input, int value){
        ...
        x = 0;
        if(value >= h){x = (value - value % h)/h;	value = value % h;}
        //System.out.print(x+" ");
        input[0] = setBits(input[0], 'h', x);
        
        x = 0;
        if(value >= g){x = (value - value % g)/g;	value = value % g;}
        //System.out.print(x+" ");
        input[0] = setBits(input[0], 'l', x);
        
        x = 0;
        if(value >= f){x = (value - value % f)/f;	value = value % f;}
        //System.out.print(x+" ");
        input[1] = setBits(input[1], 'h', x);
        
        x = 0;
        if(value >= e){x = (value - value % e)/e;	value = value % e;}
        //System.out.print(x+" ");
        input[1] = setBits(input[1], 'l', x);
        
        x = 0;
        if(value >= d){x = (value - value % d)/d;	value = value % d;}
        //System.out.print(x+" ");
        input[2] = setBits(input[2], 'h', x);
        
        x = 0;
        if(value >= c){x = (value - value % c)/c;	value = value % c;}
        //System.out.print(x+" ");
        input[2] = setBits(input[2], 'l', x);
        
        x = 0;
        if(value >= b){x = (value - value % b)/b;	value = value % b;}
        //System.out.print(x+" ");
        input[3] = setBits(input[3], 'h', x);
        
        x = 0;
        if(value >= a){x = (value - value % a)/a;	value = value % a;}
        //System.out.print(x+" ");
    ...
    /*
     * input = byte[] of length 4
     * pos is either 'l' for low or 'h' for high
     * value is between 0 and 15 (limitations of 4-bit binary)
     * the method sets the 4 bits specified to the given value
     * @author Mehul
     */
    private static byte setBits(byte input, char pos, int value){
        ...
        a = 0;
        if(value-8 >= 0){a = 1;			value = value - 8;}
        b = 0;
        if(value-4 >= 0){b = 1;			value = value - 4;}
        c = 0;
        if(value-2 >= 0){c = 1;			value = value - 2;}
        d = 0;
        if(value-1 >= 0){d = 1;}
    ...
    /*
     * input = byte[] of length 4
     * the method parses the array to get the int value it represents
     * @author Mehul
     */
    ...
    public static void download(TorrentInfo info) throws IOException,
    BencodingException {
        URL link, link2;
        ...
        Peer peer = null;
        peer = extract(peerList);
        
        // Print out peerlist FLAG
        for(int i = 0; i < peerList.size(); i++){
        	System.out.println("Peer #" + i + " peer: " + peerList.get(i));
        }
        // Print out peerlist FLAG
        
        // System.out.println("Peer Connection:	port:" + peer.port + " address:" + peer.IP +
        // " peer id:" + peer.peerID.toString());
        
        numActiveThreads = 0; // initialize
        // CREATE THREAD TO DOWNLOAD
        Downloader d1 = new Downloader(peer, info, b);
        numActiveThreads++; // One active right now
        
        d1.start(); // Run the thread and download the Rick Roll
        ...
        System.out.println("NumActiveThreads: " + numActiveThreads); // Flag
        //System.out.println("LEFT WHILE LOOP"); // fLAGG
        return;
    }
    
    /*
     * This method takes the piece sent by the peer and adds it to the file.
     * If we already have that piece, the method simply returns.
     * @author Mehul
     */
	...
	public static byte[] addPiece(byte[] new_piece) throws IOException{
        ...
        //the first 4 byte block corresponds to the hex-based index of the piece
        ...
        //the next 4 bytes correspond to the begin position(in bytes), this is also in hex
        ...
        pieces[index] = 1;
        remaining--;
        //System.out.printf("New piece added at index %d. %d pieces remaining%n",index,remaining);
    ...
    /*
     * This method returns the request message for the next piece to get.
     * When we create a request message for a piece we set its index equal to 2
     *  to indicate that we are trying to get that piece, but we don't have it yet
     * @author Mehul
     */
    public static byte[] whatNext(){
        ...
        //a '0' indicates that we do not have that piece and no thread is requesting it
        //a '2' indicates that we do not have the piece, but a thread sent a request for it
        for(x=0; x<pieces.length; x++){			//first we try to find a piece that has a '0'
            if(pieces[x]==0)
                break;
        }
        if(x==pieces.length){x--;}
        if(x==pieces.length-1 && pieces[x]!=0){	//else we try to find a piece that has a '2'
            for(x=0; x<pieces.length; x++)
                if(pieces[x]==2)
                    break;
        }
        ...
        //begin is the begin position, in bytes, where this piece would start in the file
        ...
        //(4-byte length prefix, message id, 4-byte index, 4-byte begin, 4-byte piece length)
        
    }
    
    
    /*
     * This method parses the byte array sent by the peer
     * 	to determine what kind of message has been sent.
     * @author Mehul
     */
     ...
     * 
     * @param byte[] info hash, byte[] peer_id
     * @return byte[]
     * @author Chris
     ...
     * those bytes.
     * 
     * @param int length_prefix, int messageID
     * @author Chris
     * @return Message
     */
    // this method is unfinished, for the last three cases we need to get the
    // payload for the message.
    public static Message message(int length_prefix, int message_id)
    throws UnsupportedEncodingException {
    ...
    public static Peer extract(@SuppressWarnings("rawtypes") ArrayList list)
    throws UnsupportedEncodingException {
        // use the byte buffers to search through a HashMap<ByteBuffer,Object>
        // for the right information.
        ByteBuffer RUpeer_id = ByteBuffer.wrap(new byte[] { 'p', 'e', 'e', 'r',
            ' ', 'i', 'd' });
        ...
        for (int i = 0; i < dictionary_size; i++) {
            HashMap<ByteBuffer, Object> dictionary_map = (HashMap<ByteBuffer, Object>) list
            .get(i);
            ...
            /*if (ID.contains("RU1103")) {
             break;
             }*/
            if(ip_address.contains("128.6.171.131"))
                break;
        }
    ...
    public static Object DecodeResponse(byte[] tracker_response)
    throws BencodingException {
        Object obj = null;
        try {
    ...
    /**
     * @author Manan
     * Decrements the active thread global variable
     */
    public static void decrementActiveThreads(){
        numActiveThreads--;
        System.out.println("Number of Active Threads: " + numActiveThreads); // FLAG
    }
